...
public class BingSpeech {
	...
	public void recordThis(String _textToSynthesize, int fileName) {

		String textToSynthesize = _textToSynthesize;
		...
		try {
			byte[] audioBuffer = TTSService.Synthesize(textToSynthesize,
					outputFormat, deviceLanguage, genderName, voiceName);

			// write the pcm data to the file
			String outputWave = ("soundFiles/" + fileName + ".pcm");
			...
			AudioFormat audioFormat = new AudioFormat(
					AudioFormat.Encoding.PCM_SIGNED, 16000, 16, 1, 1 * 2,
					16000, false);

			AudioInputStream audioInputStream = AudioSystem
					.getAudioInputStream(new File(outputWave));
