 ...
 *
 */
public class TextToSpeech extends PApplet{
	...
	public void saveTTSfile(String text) {
		...
		try (AudioInputStream audio = marytts.generateAudio(text)) {
			...
			try {
			    AudioSystem.write(audio, AudioFileFormat.Type.WAVE, new File("soundFiles/testText2Speech.wav")
			    );
			  } 
			  catch(Exception e) {
			    e.printStackTrace();
	...
	public void saveTTSfile(String text, int index) {
		...
		try (AudioInputStream audio = marytts.generateAudio(text)) {
			...
			// used has to be initiated every time
			try {
			    AudioSystem.write(audio, AudioFileFormat.Type.WAVE, new File("soundFiles/" + index + ".wav")
			    );
			  } 
			  catch(Exception e) {
			    e.printStackTrace();
...
public void saveAIFfile(String text, int index) {
		...
		try (AudioInputStream audio = marytts.generateAudio(text)) {
			...
			// used has to be initiated every time
			try {
			    AudioSystem.write(audio, AudioFileFormat.Type.AIFF, new File("soundFiles/" + index + ".aif")
			    );
			  } 
			  catch(Exception e) {
			    e.printStackTrace();
	...
	public void playTTSfile(){
		SoundFile sf = new SoundFile(this, "testText2Speech.wav");
		sf.play(1.0f, 1.0f);
		sf.play(2.0f, 1.0f);
	}
	
	public void playTTSfile(int index){
		SoundFile sf = new SoundFile(this, index + ".wav");
		sf.play(1.0f, 1.0f);
		sf.play(2.0f, 1.0f);
	}
